...
workFP = np.sum(E*dt/2*(p     / lorentz_p),    axis=1) * pCharge   #energy gain via E in first push
workSP = np.sum(E*dt/2*(pplus / lorentz_pplus),axis=1) * pCharge  #energy gain via E in second push

workE_boris = workFP+workSP  #total energy gain via E, obtained from boris velocity, slightly different from sim velocity
"""
#----------------------------------------------
...
plt.plot(t[1:]-dt/2,workE_basic,label=r'$\mathbf{v} \cdot \mathbf{E}_{basics}$',linestyle="-")
plt.plot(t-dt/2,workE_boris,label=r'$\mathbf{v} \cdot \mathbf{E}_{boris}$',linestyle="-")
