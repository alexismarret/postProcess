...
#----------------------------------------------
#CS3Drmhr
rcamera=np.array([-465,-300,280])*muFac  #c/wpi
fov = (.45,.45)
...
for i in range(0,len(time)):
    fig = plf.plotPathTrace([o.path+'/MS/DENSITY/iL/charge'],['charge'],i,
                  rcamera=rcamera,
                  thcamera=thcamera,
                  pixels=pixels,
                  image=image,
                  vlabel=r"$n_{iL}\ [en_ec]$",
                  # xlim=xlim,ylim=ylim,zlim=zlim,
                  dr=0.5,
                  figsize=(5,4),dpi=200,opacity=2,    #opacity : physical length x opacity x value of pixel =1
                  vlim=(0.1,1.),cmap='bwr',fov=fov,
                  cbarloc='center right',
                  # tickparams=([[10,5],[10,5],[10,5]]),  #CS3Dkink
                  tickparams=([[5,2],[5,2],[5,2]]),
