...
for j in range(len(run)-1):
    ...
    sub1.axvline(time[indKinkSat],color=colors[j],linewidth=0.7)
...
for j in range(len(run)-1):
    ...
    sub1.plot(time, Te[j],color=colors[j],linestyle="--")
    sub1.plot(time, Ti[j],color=colors[j],label=run[j])
...
sub1.plot(time, Te_mu32,color="k",linestyle="--")
sub1.plot(time, Ti_mu32,color="k",label="CS3Dmu32noB")

sub1.set_xlabel(r"$t\ [\omega_{pi}^{-1}]$")
sub1.set_ylabel(r"$T\ [T_0]$")
sub1.legend(frameon=False)
sub1.set_yscale("log")
