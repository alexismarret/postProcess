...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul  7 10:01:49 2022
...
data = (o.getTotCurrent(time[0], "y", sl=sl) * o.getB(time[0], "z", sl=sl) -
        o.getTotCurrent(time[0], "z", sl=sl) * o.getB(time[0], "y", sl=sl)) / (-o.getCharge(time[0],"eL",sl=sl)*2)
...
for i in range(len(time)):
    ...
    data = (o.getTotCurrent(time[i], "y", sl=sl) * o.getB(time[i], "z", sl=sl) -
            o.getTotCurrent(time[i], "z", sl=sl) * o.getB(time[i], "y", sl=sl)) / (-o.getCharge(time[i],"eL",sl=sl)*2)
    if doFilter: o.low_pass_filter(data, cutoff=cutoff, axis=axf)
