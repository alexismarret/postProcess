...
for i in range(len(mi)):
    ...
    bsat = gamma**2*gammaFil**2*mi[i] / (vi*gamma*kFil*2*np.pi) #trapping

    epsB = bsat**2/2

    epsB_classical = 1/2 /(2*np.pi)**2 * n0*mi[i]*vi**2  #classical trapping
    ...
    #from Faraday's law
    alpha = 2/3
    Te_if = vi**2*bsat*alpha*(mi[i]/me)**(3/4.) /wpi
...
# sub1.plot(vi,np.sqrt(p2c2),color="orange",linestyle="--")
