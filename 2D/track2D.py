...
params={'axes.titlesize' : 9, 'axes.labelsize' : 9, 'lines.linewidth' : 0.9,
        'lines.markersize' : 0.5, 'xtick.labelsize' : 9, 'ytick.labelsize' : 9,
...
if mult:
    ...
    sub2 = fig.add_subplot(gs[1,0], sharex=sub1)
    ...
    sub1.set_ylim(np.min(ene),np.max(ene))


    sub1.set_ylabel(r'$\mathcal{E}\ [m_ec^2]$')
    sub2.set_ylabel(r'$\mathcal{E}\ [m_ec^2]$')
    ...
    sub2.plot(t[imax],intWork2[imax],color="r",linestyle="dotted",
              label=r"$\max(\mathrm{q}\int\bf{v}\cdot\bf{E}_y \mathrm{dt})$")

    sub2.plot(t[0],np.mean(intWork1,axis=0),color="b",linestyle="dashed",
...
if position:
    ...
    x1  = tr.getTrackData(o.path,species,"x1" )[sl] / np.sqrt(np.abs(muNorm))
    x2  = tr.getTrackData(o.path,species,"x2" )[sl] / np.sqrt(np.abs(muNorm))
    ...
    sub1.set_xlabel(r'$x\ [c/\omega_{pi}^{-1}]$')
    sub1.set_ylabel(r'$y\ [c/\omega_{pi}^{-1}]$')

    o.setup_dir(o.path+"/plots/trackPos")

    txt = sub1.text(0.35, 1.05,
                    r"$t=%.1f\ [\omega_{pi}^{-1}]$"%t[0,0],
                    horizontalalignment='right',
                    verticalalignment='bottom',
                    transform=sub1.transAxes)

    plt.savefig(o.path+"/plots/trackPos"+"/plot-{i}-time-{t}.png".format(i=0,t=t[0,0]),dpi="figure")

    for i in range(len(t[0])):
        im = sub1.scatter(x1[:,i],x2[:,i],color="k")

        Artist.remove(txt)
        txt = sub1.text(0.35, 1.05,
                        r"$t=%.1f\ [\omega_{pi}^{-1}]$"%t[0,i],
                        horizontalalignment='right',
                        verticalalignment='bottom',
                        transform=sub1.transAxes)

        plt.pause(pause)
        plt.savefig(o.path+"/plots/trackPos"+"/plot-{i}-time-{t}.png".format(i=i,t=t[0,i]),dpi="figure")
        im.remove()
